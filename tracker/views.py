from django.shortcuts import render, redirect
from django.contrib.auth import login, authenticate, logout
from django.contrib.auth.forms import AuthenticationForm
from django.contrib.auth.decorators import login_required
from .forms import RegisterForm, IncomeForm, ExpenseForm, EMIForm
from .models import Income, Expense, EMI, Category

# Register view
def register(request):
    if request.method == 'POST':
        form = RegisterForm(request.POST)
        if form.is_valid():
            user = form.save()
            login(request, user)
            Category.create_default_categories(user)  # Create default categories after registration
            return redirect('dashboard')
    else:
        form = RegisterForm()
    return render(request, 'tracker/register.html', {'form': form})

# Login view
def login_view(request):
    if request.method == 'POST':
        form = AuthenticationForm(request, data=request.POST)
        if form.is_valid():
            user = form.get_user()
            login(request, user)
            return redirect('dashboard')
    else:
        form = AuthenticationForm()
    return render(request, 'tracker/login.html', {'form': form})

# Logout view
def logout_view(request):
    logout(request)
    return redirect('login')

# Add Income view
@login_required
def add_income(request):
    if request.method == 'POST':
        form = IncomeForm(request.POST)
        if form.is_valid():
            income = form.save(commit=False)
            income.user = request.user  # Associate the income with the logged-in user
            income.save()
            return redirect('dashboard')
    else:
        form = IncomeForm()

    # Fetch income categories for the logged-in user
    categories = Category.objects.filter(type='income', user=request.user)

    return render(request, 'tracker/add_income.html', {'form': form, 'categories': categories})

# View Incomes
@login_required
def view_incomes(request):
    incomes = Income.objects.filter(user=request.user)
    return render(request, 'tracker/view_incomes.html', {'incomes': incomes})

# Add Expense view
@login_required
def add_expense(request):
    if request.method == 'POST':
        form = ExpenseForm(request.POST)
        if form.is_valid():
            expense = form.save(commit=False)
            expense.user = request.user  # Associate the expense with the logged-in user
            expense.save()
            return redirect('dashboard')
    else:
        form = ExpenseForm()

    # Fetch expense categories for the logged-in user
    categories = Category.objects.filter(type='expense', user=request.user)

    return render(request, 'tracker/add_expense.html', {'form': form, 'categories': categories})

# View Expenses
@login_required
def view_expenses(request):
    expenses = Expense.objects.filter(user=request.user)
    return render(request, 'tracker/view_expenses.html', {'expenses': expenses})

# Dashboard view
@login_required
def dashboard(request):
    incomes = Income.objects.filter(user=request.user)
    expenses = Expense.objects.filter(user=request.user)
    emis = EMI.objects.filter(user=request.user)

    total_income = sum(income.amount for income in incomes)
    total_expenses = sum(expense.amount for expense in expenses)
    balance = total_income - total_expenses

    context = {
        'incomes': incomes,
        'expenses': expenses,
        'emis': emis,
        'total_income': total_income,
        'total_expenses': total_expenses,
        'balance': balance,
    }

    return render(request, 'tracker/dashboard.html', context)

# Home view (redirects to login or dashboard)
def home(request):
    if request.user.is_authenticated:
        return redirect('dashboard')
    else:
        return redirect('login')

# Add EMI view
@login_required
def add_emi(request):
    if request.method == 'POST':
        form = EMIForm(request.POST)
        if form.is_valid():
            emi = form.save(commit=False)
            emi.user = request.user
            emi.save()
            return redirect('dashboard')
    else:
        form = EMIForm()

    return render(request, 'tracker/add_emi.html', {'form': form})
